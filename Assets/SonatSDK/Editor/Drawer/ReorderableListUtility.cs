...
public abstract class BaseReorderableEditor : Editor
{
    ...
    protected abstract string[] ListNames { get; }

    protected virtual void OnEnable()
    {
        _lists = new ReorderableList[ListNames.Length];
        for (var i = 0; i < _lists.Length; i++)
        {
            _lists[i] = ReorderableListUtility.CreateAutoLayout(serializedObject.FindProperty(ListNames[i]));
        }
    }
}
...
public static class ReorderableListUtility
{
    ...
    public static void CreateEnumNameList(SerializedProperty arrayProp, Type enumType)
    {
        ...
        if (arrayProp.isExpanded)
        {
            ...
            var values = ((int[]) Enum.GetValues(enumType)).ToList();
            ...
            for (int i = 0; i < arrayProp.arraySize; ++i)
            {
                ...
                if (index >= 0)
                {
                    EditorGUILayout.PropertyField(transformProp, new GUIContent(i+":"+names[index]));
                }
                else
                {
                    EditorGUILayout.PropertyField(transformProp, new GUIContent(i+":[not defined]"));
                }
        ...
        return (rect, index, isActive, isFocused) =>
        {
            ...
            for (var ii = 0; ii < columns.Count; ii++)
            {
                ...
                if (ii == 0)
                {
                    int labelWidth = 15;
                    var copy1 = arect;
                    copy1.width = labelWidth;
                    EditorGUI.LabelField(copy1, index.ToString());
                    var copy = arect;
                    copy.width -= labelWidth;
                    copy.x += labelWidth;
                    EditorGUI.PropertyField(copy, property.GetArrayElementAtIndex(index).FindPropertyRelative(c.PropertyName), GUIContent.none);
